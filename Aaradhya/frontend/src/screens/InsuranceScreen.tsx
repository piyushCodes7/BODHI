...
// Text-based icons for story slides — no emojis
...
async function pickPdfFile(): Promise<{ name: string; uri: string } | null> {
  // Replace with real expo-document-picker in production:
  // const result = await DocumentPicker.getDocumentAsync({ type: 'application/pdf' });
  // if (result.canceled) return null;
  // return { name: result.assets[0].name, uri: result.assets[0].uri };
  return { name: 'demo_policy.pdf', uri: 'mock://demo' };
}
